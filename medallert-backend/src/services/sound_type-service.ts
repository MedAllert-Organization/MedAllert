...
export class SoundTypesService {
  constructor(private readonly soundTypesRepository: SoundTypesRepository) {}

  async create({ sound }: SoundTypesType): PromiseResult<SoundTypes> {
  ...
  async update(
    soundId: string,
    updateData: Partial<SoundTypesType>,
  ): PromiseResult<SoundTypes> {
    const sound = await this.soundTypesRepository.findSoundType(soundId);
    if (!sound) return error("Sound not found");
    ...
    if (!updatedOk || !updatedSound)
      return error("Failed to update sound type");
  ...
  async delete(soundId: string): PromiseResult<SoundTypes> {
    ...
    if (!deletedOk || !deletedSound)
      return error("Failed to delete sound type");
