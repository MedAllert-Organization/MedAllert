...
export class ReportService {
  ...
  constructor(
   ...
   async generateReport(userId: string, period: "Weekly" | "Monthly"): Promise<string> {
    const now = new Date();
    ...
    const UserTreatment = await this.treatmentRepository.findAllTreatments(userId);
        ...
        if (treatments.length > 0) {
          ...
          treatments.forEach((treatment, index) => { 
            doc.fontSize(10)
               ...
               .text(`   Descrição: ${treatment.description || "Nenhuma descrição"}`)
               .text(`   Início: ${treatment.startAt.toLocaleDateString('pt-BR')}`)
               .text(`   Término: ${treatment.endAt ? treatment.endAt.toLocaleDateString('pt-BR') : "Em andamento"}`)
          ...
          doc.fontSize(10)
             ...
             .text(`Tratamentos em andamento: ${treatments.filter(t => !t.endAt).length}`)
             .text(`Tratamentos concluídos: ${treatments.filter(t => t.endAt && t.endAt <= endDate).length}`);

        } else {
          doc.fontSize(12).text("Nenhum tratamento encontrado para o período selecionado.");
        }
