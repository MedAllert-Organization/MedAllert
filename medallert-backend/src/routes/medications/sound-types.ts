...
import {
  SoundTypesIdParamSchema,
  SoundTypesSchema,
  SoundTypesService,
  SoundTypesUpdateSchema,
} from "../../services/sound_type-service.js";

export const soundTypes = new Hono();

const soundTypesService = new SoundTypesService(defaultSoundTypesRepository);
...
soundTypes.post(
  ...
  async (c) => {
    ...
    const [ok, error, soundType] =
      await soundTypesService.create(soundTypeCandidate);
...
soundTypes.put(
  ...
  async (c) => {
    ...
    const [ok, error, updatedSoundType] = await soundTypesService.update(
      id,
      updateData,
    );
