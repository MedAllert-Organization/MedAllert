...
import { NotificationIdParamSchema, NotificationSchema, NotificationService, NotificationUpdateSchema } from "../../services/notification-service.js";
...
notification.post(
    ...
    async (c) => {
        ...
        const [ok, error, notification] = await notificationService.create(notificationCandidate);
...
notification.put(
    ...
    async (c) => {
        ...
        const [ok, error, updatedNotification] = await notificationService.update(id, updateData);

        if (!ok || !updatedNotification) return c.json({ success: false, error }, 404);
...
notification.delete(
    ...
    async (c) => {
        const { id } = c.req.valid("param");
        const [ok, error, deletedNotification] = await notificationService.delete(id);
        if (!ok || !deletedNotification) return c.json({ success: false, error }, 404);
